...
function updateStatuses(user, statuses, shouldSort=true) {
    ...
    for (var i = 0; i < statuses.length; i++) {
        ...
        let content_html = "";
        if (statuses[i]["reblog"]) {
            content_html += "<span title=\"" + statuses[i]["account"]["username"] + " boosted\">&#128640;</span> " + statuses[i]["reblog"]["account"]["username"] + "<br><a style=\"padding-left:36px\" href=\"" + statuses[i]["reblog"]["account"]["remote_actor"] + "\">@" + statuses[i]["reblog"]["account"]["acct"] + "</a><br>";
        } else {
